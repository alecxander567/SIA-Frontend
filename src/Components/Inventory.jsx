import { FaTachometerAlt, FaBox, FaClipboardList, FaChartBar, FaBell, FaSignOutAlt, FaSearch, FaCog,  FaBars, FaTimes, FaChevronLeft, FaChevronRight, FaPlus } from "react-icons/fa";
  ...
  const handleLogout = async () => {
    ...
    return(
        ...
        <div className="bg-white shadow px-10 py-4 relative flex items-center">
          ...
          >
            {["All Categories", "Tools", "Electronics Supplies", "Plumbing Supplies"].map((category) => (
              <div
                key={category}
                className="cursor-pointer bg-gray-200 px-4 py-2 rounded-full hover:bg-black hover:text-white transition"
              >
                {category}
              </div>
            ))}
        ...
        <div className="px-5 py-4 flex flex-col gap-6">
          {/* First row: Add button + first 3 items */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <button
              onClick={() => setShowModal(true)}
              className="w-full h-60 flex flex-col items-center justify-center text-blue-500 border-4 border-dashed border-blue-500 bg-transparent hover:bg-blue-500 hover:text-white transition text-lg font-semibold rounded-[10px]"
            >
            ...
            {items.slice(0, 3).map((item, index) => (
              <div
                key={index}
                className="bg-white shadow rounded-[10px] p-4 flex flex-col gap-2"
              >
                {item.imagePath?.trim() ? (
                  <img
